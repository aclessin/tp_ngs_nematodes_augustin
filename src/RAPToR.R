...
plot(comparaison$ref$coefficients[2,],comparaison$samp$coefficients[2,],
     ylim=c(-5,5),xlim=c(-5,5),col=1,cex=0.4,xlab='log2fold ref',ylab='log2fold sample',main="alg-1")

points(comparaison$ref$coefficients[2, comparaison$ovl_genelist %in% upgenes],
       comparaison$samp$coefficients[2,comparaison$ovl_genelist %in% upgenes],col=2,cex=0.8)
points(comparaison$ref$coefficients[2, comparaison$ovl_genelist %in% downgenes],
       comparaison$samp$coefficients[2,comparaison$ovl_genelist %in% downgenes],col=3,cex=0.8)
...
legend('bottomright',legend=c("upregulated","downregulated","not significant"),
       col=c(3,2,1), pch=1, lwd=2, lty=NA,x.intersp = 0.05, bty="n")
